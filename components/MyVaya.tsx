...
// Fixed: Using $visible instead of visible to make it a transient prop
...
const LastBookingTitle = styled.h3`
  color: ${luxuryPalette.textMuted};
  font-size: 0.9em;
  margin-bottom: 10px;
...
const LastBookingDetails = styled.p`
  color: ${luxuryPalette.text};
  font-size: 1.1em;
  margin: 5px 0;
...
const MyVaya: React.FC<{ savings: Savings }> = ({ savings }) => {
  ...
  return (
    <Dashboard>
      {/* Fixed: Using $visible instead of visible */}
      ...
      {savings.lastBooking && (
        <LastBooking>
          <LastBookingTitle>Last Booking</LastBookingTitle>
          <LastBookingDetails>
            <span>{savings.lastBooking.destination}</span> - Saved ${savings.lastBooking.saved}
          </LastBookingDetails>
          <LastBookingDetails>
            {new Date(savings.lastBooking.date).toLocaleDateString("en-US", {
              year: "numeric",
              month: "long",
              day: "numeric",
            })}
